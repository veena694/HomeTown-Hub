...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-[#FFFDF7]/90 dark:bg-[#18201C]/90 backdrop-blur-md border-b border-hub-border shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-hub-cream border border-hub-terracotta/40 hover:border-hub-terracotta text-xs font-semibold text-hub-terracotta flex items-center gap-1.5 shadow-2xs transition-all"
            >
              <MapPin className="w-3.5 h-3.5 text-hub-terracotta animate-pulse" />
              <span className="font-mono">
                {currentLocation ? `${currentLocation.city}, ${currentLocation.state}` : 'Select Hometown'}
              </span>
              <ChevronDown className="w-3.5 h-3.5 text-hub-sage" />
            </button>

            {/* LOCATION SWITCHER DROPDOWN MENU */}
            {isLocationDropdownOpen && (
              <div className="absolute top-full left-0 mt-2 w-84 rounded-2xl bg-white dark:bg-[#27322B] border border-hub-border p-4 shadow-xl z-50 text-hub-charcoal space-y-4 animate-accordion-down">
                {/* Search Box */}
                <div className="relative">
                  <Search className="w-3.5 h-3.5 text-hub-sage absolute left-3 top-1/2 -translate-y-1/2" />
                  <input
                    type="text"
                    placeholder="Search another location to explore..."
                ...
                <button
                  ...
                  className="w-full p-2.5 rounded-xl bg-hub-green/10 hover:bg-hub-green/20 border border-hub-green/30 text-hub-charcoal text-xs font-semibold flex items-center justify-between transition-colors"
                >
                ...
                <div className="p-3 rounded-xl bg-hub-stone border border-hub-border space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="text-[11px] text-hub-terracotta font-semibold hover:underline flex items-center gap-1"
                    >
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="p-2 rounded-lg bg-hub-terracotta/10 hover:bg-hub-terracotta/20 border border-hub-terracotta/30 text-hub-terracotta text-xs font-semibold flex items-center gap-1.5 truncate"
                    >
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg bg-hub-sky/20 hover:bg-hub-sky/30 border border-hub-sky/40 text-hub-charcoal text-xs font-semibold flex items-center gap-1.5 truncate"
                    >
        ...
        {/* User Actions, Theme Switcher & Search Button */}
        <div className="hidden md:flex items-center gap-3">
          {/* THEME TOGGLE BUTTON */}
          <button
            onClick={toggleTheme}
            className="p-2 rounded-xl bg-hub-stone hover:bg-hub-border text-hub-charcoal transition-colors border border-hub-border flex items-center gap-1.5 text-xs"
          ...
          <Link
            href="/notifications"
            className="relative p-2 rounded-xl bg-hub-stone hover:bg-hub-border text-hub-charcoal transition-colors border border-hub-border"
          >
        ...
        <button
          ...
          className="md:hidden p-2 rounded-xl bg-hub-stone text-hub-charcoal"
        >
